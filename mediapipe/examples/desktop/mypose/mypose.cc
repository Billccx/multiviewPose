...
absl::Status RunMPPGraph() {
  ...
  ASSIGN_OR_RETURN(mediapipe::OutputStreamPoller landmarks_poller,graph.AddOutputStreamPoller("pose_landmarks"));
  ...
  while (grab_frames) {
    ...
    mediapipe::Packet packet,landmarks_packet;
    if (!poller.Next(&packet)) break;
    auto& output_frame = packet.Get<mediapipe::ImageFrame>();
    
    if (!landmarks_poller.Next(&landmarks_packet)) break;
    printf("QueueSize: %d\n", landmarks_poller.QueueSize());    //<<== QueueSize() always is zero. Why?
