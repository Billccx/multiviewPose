...
namespace mediapipe{
    ...
    class UnpackerCalculator : public CalculatorBase {
        public:
        int unpackercnt;
        rs2::align align_to_color;
        std::chrono::_V2::steady_clock::time_point start,now;

        UnpackerCalculator():align_to_color(RS2_STREAM_COLOR){
            unpackercnt=0;
        }
        ...
        absl::Status Process(CalculatorContext* cc) override{
            ...
            // rs2::video_frame color=frmset.get_color_frame();
            // rs2::depth_frame depth=frmset.get_depth_frame();

            cv::Mat color_mat=frame_to_mat(color);

            cv::imwrite(
                "/home/cuichenxi/mediapipe/mediapipe/examples/desktop/mypose5/data/fold1/"
            ...
            // cc->Outputs().Tag(depthTag).AddPacket(
            //     mediapipe::MakePacket<rs2::depth_frame>(depth).At(cc->InputTimestamp())
            // );

            return absl::OkStatus();
        }
    };
    REGISTER_CALCULATOR(UnpackerCalculator);
