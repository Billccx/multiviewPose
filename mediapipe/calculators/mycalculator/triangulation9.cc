...
namespace mediapipe{
    ...
    class TriangulationCalculator9 : public CalculatorBase {
        public:
        int gcnt;
        int udp_socket; 
        const int port;
        ...
        TriangulationCalculator9():gcnt(0),port(31201){
            intrin0.width=640;
            intrin0.height=480;
            intrin0.ppx=331.948;
            intrin0.ppy=248.697;
        ...
        absl::Status Process(CalculatorContext* cc) override{
            ...
            memset(points2d_gt,0,sizeof(points2d_gt));
            ...
            for(int i=0;i<landmark_sz;i++){
                ...
                points2d_gt[i][0]=(float)(x0*640);
                points2d_gt[i][1]=(float)(y0*480);
                vis0_gt[i]=v0;
                if(points2d_gt[i][0]>=0 && points2d_gt[i][0]<=640 && points2d_gt[i][1]>=0 && points2d_gt[i][1]<=480){
                    depth_gt[i] = ptrdepth0.get_distance(points2d_gt[i][0],points2d_gt[i][1]);
                    rs2_deproject_pixel_to_point(points3d_gt[i],&intrin0,points2d_gt[i],depth_gt[i]);
                }
            ...
            std::ofstream outfile("/home/cuichenxi/mediapipe/mediapipe/examples/desktop/mypose10/data2/"+
    ...
    REGISTER_CALCULATOR(TriangulationCalculator9);
